...
- `composer ic:kb:build [paths...]` - explicitly build or reuse the deterministic project bundle under `phpforge/`. PHP is structurally extracted; other UTF-8 files are content-indexed. `analysis.json`, `GRAPH_REPORT.md`, and `graph.html` include graph-derived communities, cohesion, god nodes, extracted cross-community connections, health diagnostics, and suggested questions; never present those derived rankings as new source facts.
- `composer ic:kb:query -- <question> [options]` - retrieve a depth-aware local graph slice with lexical seed ranking, intent/relation filtering, BFS or DFS traversal, token budgeting and architectural community/hub/bridge context. The Composer `--` separator is required before command arguments. Use `--depth`, `--context`, `--relation`, `--direction`, `--dfs`, `--budget` and `--limit` to tune retrieval. `--explain` prefers the active Ollama model and falls back to Gemini when `GEMINI_API_KEY` is set. Use `--provider=ollama` for local-only operation or `--provider=gemini` for explicit remote use. AI explanations must remain explicitly labelled inferred.
...
- Project knowledge builds are model-free and network-free. PHPProbe owns extracted PHP facts; PHPForge owns persistence, content indexing and retrieval. Treat every model explanation as inferred and never let it replace extracted facts. A configured `GEMINI_API_KEY` authorizes automatic Gemini fallback when Ollama is unavailable; use `--provider=ollama` when remote transmission is prohibited.
